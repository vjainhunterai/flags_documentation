...
def main():
    # --- Load data ---
    print("Loading data...")
    df_cchs = pd.read_excel(CCHS_FILE, engine='openpyxl')
    df_umass = pd.read_excel(UMASS_FILE, engine='openpyxl')
    ...
    # --- Greedy 1:1 matching (no duplicates on either side) ---
    # Sort all candidate pairs by Combined_Score descending, then greedily
    # assign each pair only if neither CCHS nor UMass item is already taken.
    print("Assigning unique 1:1 matches (greedy)...")
    df_sorted = df_results.sort_values('Combined_Score', ascending=False).reset_index(drop=True)
    used_cchs = set()
    used_umass = set()
    best_rows = []

    for _, row in df_sorted.iterrows():
        cchs_key = row['CCHS_Description']
        umass_key = row['UMass_Description']
        if cchs_key not in used_cchs and umass_key not in used_umass:
            best_rows.append(row)
            used_cchs.add(cchs_key)
            used_umass.add(umass_key)
    ...
    # Top-3 candidates per CCHS item (for reference/review)
    df_top3 = (
        df_results
    ...
    print(f"\nMatch Summary (1:1 unique):")
    print(df_best['Confidence'].value_counts().to_string())
    print(f"Total unique pairs: {len(df_best)}")
